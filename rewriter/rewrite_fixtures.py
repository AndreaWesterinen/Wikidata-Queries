...
def discover_fixtures(root: Path) -> list[tuple[Path, Path]]:
    """Find original queries recursively and determine the output paths."""
...
def main() -> int:
    """Run all fixtures and write generated output outside the test directory ('/generated') by default."""
    ...
    destinations.add_argument("--output-dir", type=Path,
        default=Path(__file__).resolve().parent / "generated",
        help="Generated output root (default: generated)")
    destinations.add_argument("--update-golden", action="store_true",
    ...
    for original, _ in fixtures:
        ...
        rewritten_path = destination_path(
            original, args.fixtures, args.output_dir, args.update_golden)
